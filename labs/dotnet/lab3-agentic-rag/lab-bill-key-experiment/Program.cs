...
class Program
{
    ...
    static async Task DemoModeAsync()
    {
        ...
        Console.WriteLine("\n[1/5] Loading Foundry configuration...");
        FoundryClientConfiguration foundryConfig;
        try
        {
            foundryConfig = FoundryClientFactory.GetConfiguration();
            Console.WriteLine($"✓ Foundry config loaded (Endpoint: {foundryConfig.Endpoint})");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"✗ Foundry configuration error: {ex.Message}");
            return;
        }
        ...
        try
        {
            FoundryClientFactory.ValidateSearchConfiguration(foundryConfig);
            Console.WriteLine("✓ Azure Search configuration loaded");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"✗ Search configuration error: {ex.Message}");
            return;
        }

        // Initialize Azure OpenAI clients via Foundry
        Console.WriteLine("\n[2/5] Initializing Azure OpenAI client via Foundry...");
        var chatClient = await FoundryClientFactory.CreateChatClientAsync(foundryConfig);
        var embeddingClient = await FoundryClientFactory.CreateEmbeddingClientAsync(foundryConfig);
        Console.WriteLine("✓ Chat and embedding clients initialized");

        // Initialize search service
        Console.WriteLine("\n[3/5] Initializing Azure AI Search service...");
        var searchClient = new SearchClient(
            new Uri(foundryConfig.SearchEndpoint!),
            foundryConfig.SearchIndexName!,
            new AzureKeyCredential(foundryConfig.SearchApiKey!)
        );
        var searchService = new SearchService(searchClient, chatClient, embeddingClient);
    ...
    static async Task InteractiveModeAsync()
    {
        ...
        // Initialize system with Foundry credentials
        var foundryConfig = FoundryClientFactory.GetConfiguration();
        FoundryClientFactory.ValidateSearchConfiguration(foundryConfig);
